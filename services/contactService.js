...
export const handleFileAction = function (command, args) {
  switch (command) {
    case "add":
...
export const searchContactsByEmail = function (email) {
  ...
  if (usersWithTheSameEmail.length === 0) {
    return {
      success: true,
      messageArr: [...loadResult.messageArr, `No contacts found matching "${email}"`],
      data: [],
    };
  }
  ...
  return {
    success: true,
    messageArr: [...loadResult.messageArr, `Found ${usersWithTheSameEmail.length} contact(s) matching: ${email}`],
    data: usersWithTheSameEmail,
  };
};

export const deleteContact = function (email) {
  const loadResult = listContacts();
  ...
  if (!contactToDelete) {
    return {
      success: false,
      messageArr: [...loadResult.messageArr, `Contact with email ${email} was not found`],
      data: [],
    };
  }
  ...
  if (!saveResult.success) {
    return {
      success: false,
      messageArr: [...loadResult.messageArr, `Error: ${saveResult.message}`],
      data: [],
    };
  }
  ...
  return {
    success: true,
    messageArr: [...loadResult.messageArr, `Contact deleted: ${contactToDelete.name}`, "Contacts saved to contacts.json"],
    data: [],
  };
};

export const listContacts = function () {
...
export const saveContacts = function (contacts) {
  const data = JSON.stringify(contacts, null, 2);
  return fileUtils.writeFile(CONTACTS_FILE, data);
};

export const isContactExist = function (contacts, contact) {
  return contacts.find((u) => u.email === contact.email);
};

export const searchContactsByName = function (name) {
  const loadResult = listContacts();
  if (!loadResult.success) {
    return {
      success: false,
  ...
  if (usersWithTheSameName.length === 0) {
    return {
      success: true,
      messageArr: [...loadResult.messageArr, `No contacts found matching "${name}"`],
      data: [],
    };
  }
  return {
    success: true,
    messageArr: [...loadResult.messageArr, `Found ${usersWithTheSameName.length} contact(s) matching: ${name}`],
...
export const addContact = function (contact) {
  ...
  if (existingContact) {
    return {
      success: false,
      messageArr: [...loadResult.messageArr, "Error: Contact with this email already exists"],
      data: [],
    };
  }
  ...
  return {
    success: true,
    messageArr: [...loadResult.messageArr, `Contact added: ${contact.name}`, "Contacts saved to contacts.json"],
    data: [],
  };
};

export const loadContacts = listContacts;
